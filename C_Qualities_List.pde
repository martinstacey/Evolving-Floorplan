...
void setupql() {
  qu = new QualitiesList (thenlist, t3pos);
}
void drawql() {
  qu.display();
}
...
class QualitiesList {
  ...
  ArrayList <Textedit>   iq1, iq2, iq3, iq4, iq5;
  ArrayList <Textedit>   iw1, iw2, iw3, iw4, iw5;
  IntList  a = new IntList ();
  QualitiesList( ArrayList <Node> _ntx, PVector _pos) {
    ntx = _ntx;
    if (ntx.size()>0) for (int i=0; i<ntx.size(); i++) if (ntx.get(i).room) leafs.add(ntx.get(i));
    if (ntx.size()>0) for (int i=0; i<ntx.size(); i++) if (ntx.get(i).room)  a.append(i);
    inpos=_pos;
    ...
    if (leafs.size()>0)  for (int i=0; i<leafs.size(); i++) if (leafs.get(i).idealqualities!=null) if (leafs.get(i).idealqualities.size()!=0) {
      ...
      iq1.add(new Textedit(new PVector (inpos.x+size.x*5, inpos.y+size.y*i), size, nf(leafs.get(i).idealqualities.get(0), 0, 0), "", 10));
      iq2.add(new Textedit(new PVector (inpos.x+size.x*6, inpos.y+size.y*i), size, nf(leafs.get(i).idealqualities.get(1), 0, 0), "", 10));
      iq3.add(new Textedit(new PVector (inpos.x+size.x*7, inpos.y+size.y*i), size, nf(leafs.get(i).idealqualities.get(2), 0, 0), "", 10));
      iq4.add(new Textedit(new PVector (inpos.x+size.x*8, inpos.y+size.y*i), size, nf(leafs.get(i).idealqualities.get(3), 0, 0), "", 10));
      String adjacents = "";
      for (int j=0; j<leafs.get(i).idealadjacent.size()-1; j++)  adjacents = adjacents + leafs.get(i).idealadjacent.get(j) + ",";
      adjacents = adjacents + leafs.get(i).idealadjacent.get(leafs.get(i).idealadjacent.size()-1);
